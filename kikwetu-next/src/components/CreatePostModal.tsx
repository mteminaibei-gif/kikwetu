      ...
      <div className="w-full sm:max-w-lg bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl shadow-2xl"
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-4 gap-1.5">
            {POST_TYPES.map(t => (
              <button key={t.id} onClick={() => setType(t.id)}
                className={`flex flex-col items-center gap-1 py-2.5 border-2 rounded-xl text-[10px] font-bold transition-all ${
                  type === t.id ? 'border-brand-orange bg-brand-orange/10 text-brand-orange' : 'border-gray-200 dark:border-gray-700 text-gray-500 hover:border-gray-300'
                }`}>
          ...
          <input value={title} onChange={e => setTitle(e.target.value)}
            ...
            className="w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-orange/50" />

          <textarea value={content} onChange={e => setContent(e.target.value)}
            rows={4} placeholder={type === 'question' ? 'What would you like to ask the community?' : 'Share your knowledge...'}
            className="w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-orange/50 resize-none" />

          {type === 'poll' && (
            <div className="space-y-2">
              {pollOptions.map((opt, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-xs font-bold text-brand-orange w-5">{i + 1}.</span>
                  <input value={opt} onChange={e => updatePollOption(i, e.target.value)}
                    placeholder={`Option ${i + 1}`}
                    className="flex-1 p-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-brand-orange/50" />
              ...
              <button onClick={addPollOption}
                className="text-xs text-brand-orange font-semibold hover:underline">+ Add option</button>
            </div>
          )}

          {type === 'swahili' && (
            <div className="p-3 rounded-xl bg-brand-orange/5 border border-brand-orange/20 text-xs text-gray-600 dark:text-gray-300">
          ...
          <div className="flex items-center justify-between">
            <select value={spaceId} onChange={e => setSpaceId(e.target.value)}
              className="text-xs bg-gray-100 dark:bg-gray-800 p-2 rounded-lg font-semibold border-none focus:outline-none focus:ring-2 focus:ring-brand-orange/50">
            ...
            <div className="flex items-center gap-2">
              <label className="text-xs text-gray-400 cursor-pointer hover:text-brand-orange transition-colors">
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex justify-end gap-3">
          ...
          <button onClick={handleSubmit} disabled={loading || !content.trim()}
            className="bg-brand-orange hover:bg-brand-lightOrange text-white px-6 py-2 rounded-full text-xs font-bold shadow-md transition-all disabled:opacity-50">
