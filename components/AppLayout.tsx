...
import LiveRightSidebar from '@/components/LiveRightSidebar';
import {
  ...
  X, Users, MessageCircle, ThumbsUp, AtSign, UserPlus, Award, Calendar,
  AlertTriangle, CheckCheck, Menu, Newspaper, Home
} from 'lucide-react';
...
const Topbar = React.memo(function Topbar() {
  const { theme, toggleTheme, lang, toggleLang, user, setUser, unreadCount, setUnreadCount, showToast, setSidebarOpen } = useApp();
...
const SPACE_COLORS = ['var(--greenSoft)', 'var(--blueSoft)', 'var(--redSoft)', 'var(--goldSoft)', 'var(--earthSoft)'];

const LeftSidebar = React.memo(function LeftSidebar({ activeRoute }: { activeRoute: string }) {
  const { user, showToast } = useApp();
  const router = useRouter();
  const heshima = user?.heshima || 0;
  const level = Math.floor(heshima / 100);
  const toNext = 100 - (heshima % 100 || 100);
  ...
  useEffect(() => {
    const loadSpaces = async () => {
      const { data } = await supabase
        ...
        .limit(4);
      if (data) {
        setSpacesList(data.map((s: any, i: number) => ({
          id: s.id,
          name: s.name,
          emoji: s.icon || '🏘️',
    ...
    void loadSpaces();
    const channel = supabase.channel('sidebar-spaces')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'spaces' }, () => { void loadSpaces(); })
      .subscribe();
    return () => { void supabase.removeChannel(channel); };
  }, []);
    ...
    <aside className="sidebar left-sidebar">
      ...
      <div className="sidebar-section">
        ...
        <div className="sidebar-spaces">
          {spacesList.length === 0 ? (
            ...
            <Link key={space.id} href={`/spaces/${space.id}`} className="sidebar-space-item">
      ...
      <div className="sidebar-section">
        <div className="sidebar-section-head">
          ...
          <h4>Online Now</h4>
          <span className="online-count">{onlineUsers.length + (user ? 1 : 0)}</span>
        </div>
      ...
      <div className="sidebar-section">
        <div className="sidebar-ad-card">
          ...
          <div className="sidebar-ad-content">
            ...
            <span>Join spaces and learn with communities across Kenya</span>
          </div>
        </div>
      </div>
  ...
  return (
    <AppContext.Provider value={{
      ...
      <div className="layout">
        ...
        <LiveRightSidebar pathname={pathname} />
      </div>
      {sidebarOpen && (
        <>
          ...
          <aside className="sidebar left-sidebar sidebar-overlay open">
            ...
            <div className="heshima">
              <div className="ring">{user?.heshima || 0}</div>
              <div>
                <strong>Heshima rating</strong>
                <span>Level {Math.floor((user?.heshima || 0) / 100)} · {100 - ((user?.heshima || 0) % 100 || 100)} to next</span>
              </div>
            </div>
