...
export default function ConsentBanner() {
  ...
  // Lock body scroll while banner is open
  useEffect(() => {
    if (!visible) return;
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-3 sm:p-4">
      ...
      <div
        ...
        className="relative bg-white dark:bg-brand-cardDark rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-lg animate-fadeUp overflow-hidden max-h-[min(90dvh,100%)] flex flex-col"
      >
        <div className="p-5 sm:p-6 space-y-4 overflow-y-auto overscroll-contain">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 id="consent-title" className="text-base font-bold">KikwetuConnect Privacy</h3>
          ...
          {expanded && (
            <div className="space-y-2.5 pt-2 border-t border-gray-100 dark:border-gray-800">
              {([
                ...
                <label key={key} className="flex items-center gap-3 p-2.5 rounded-xl active:bg-gray-50 dark:active:bg-gray-900 cursor-pointer transition-colors min-h-[48px] touch-manipulation">
        ...
        <div className="px-5 sm:px-6 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-2 flex flex-wrap gap-2 border-t border-gray-100 dark:border-gray-800 shrink-0">
          <button onClick={acceptAll}
            className="flex-1 min-w-[7rem] sun-btn px-4 py-3 rounded-xl text-xs font-bold shadow-md whitespace-nowrap min-h-[44px] touch-manipulation">
            Accept All
          </button>
          {expanded ? (
            <button onClick={acceptSelected}
              className="flex-1 min-w-[7rem] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-4 py-3 rounded-xl text-xs font-bold text-gray-700 dark:text-gray-300 active:bg-gray-50 dark:active:bg-gray-700 transition-all whitespace-nowrap min-h-[44px] touch-manipulation">
              Save Preferences
            </button>
          ) : (
          ...
          {expanded && (
            <button onClick={declineAll}
              className="px-4 py-3 rounded-xl text-xs font-medium text-gray-400 hover:text-gray-600 transition-colors whitespace-nowrap min-h-[44px] touch-manipulation">
              Decline
            </button>
          )}
        </div>
