      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          <div className="col-span-2 md:col-span-1 space-y-4">
            <Link href="/" className="flex items-center gap-2.5 group">
              ...
              <span className="text-lg font-black font-logo text-brand-deep dark:text-white">Kikwetu<span className="text-brand-red">Connect</span></span>
            </Link>
          ...
          {Object.entries(footerLinks).map(([key, links]) => (
            <div key={key}>
              <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-4">
                {tr(
                  key === 'platform' ? 'Platform' : key === 'resources' ? 'Resources' : 'Legal',
                  key === 'platform' ? 'Jukwaa' : key === 'resources' ? 'Rasilimali' : 'Kisheria'
                )}
              </h4>
              <ul className="space-y-3">
                {links.map(l => (
                  <li key={l.href}>
                    <Link href={l.href} className="text-sm text-gray-600 dark:text-gray-400 hover:text-brand-red transition-colors">{l.label}</Link>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="mt-12 pt-8 border-t border-gray-200 dark:border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            {socials.map(s => (
              <a key={s.label} href={s.href} className="w-9 h-9 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-brand-terracotta hover:text-white transition-all active:scale-90" aria-label={s.label}>
