    ...
    <div className="min-h-screen bg-brand-bgLight dark:bg-brand-bgDark">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="text-center space-y-4 mb-16">
          ...
          <h1 className="text-4xl sm:text-5xl font-black font-logo text-brand-deep dark:text-white">{tr('Wasiliana Nasi', 'Contact Us')}</h1>
        ...
        {submitted ? (
          <div className="sun-card p-12 text-center space-y-4">
            ...
            <h2 className="text-2xl font-bold text-brand-deep dark:text-white">{tr('Asante!', 'Thank You!')}</h2>
            ...
            <button onClick={() => setSubmitted(false)} className="text-sm text-brand-red hover:underline font-semibold">{tr('Tuma ujumbe mwingine', 'Send another message')}</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="sun-card p-8 space-y-6">
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-gray-600 dark:text-gray-400 mb-1.5 block">{tr('Jina Lako', 'Your Name')} *</label>
                <input required className="w-full p-3.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-terracotta/50" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-600 dark:text-gray-400 mb-1.5 block">{tr('Barua Pepe', 'Email')} *</label>
                <input type="email" required className="w-full p-3.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-terracotta/50" />
              </div>
            </div>
            <div>
            ...
            <div>
              <label className="text-xs font-bold text-gray-600 dark:text-gray-400 mb-1.5 block">{tr('Ujumbe Wako', 'Your Message')} *</label>
              <textarea required rows={5} className="w-full p-3.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-terracotta/50 resize-none" />
            </div>
        ...
        <div className="grid sm:grid-cols-3 gap-6 mt-12">
          {[
            ...
            <div key={c.label} className="sun-card p-6 text-center">
              ...
              <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">{c.label}</h4>
